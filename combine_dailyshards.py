...
# =====================================================
# DAILY BOXOFFICE MONTHLY LOGGING
# =====================================================
DAILY_LOG_ROOT = "daily/logs"
os.makedirs(DAILY_LOG_ROOT, exist_ok=True)

# folder: daily/logs/2026-01
MONTH_FOLDER = os.path.join(DAILY_LOG_ROOT, NOW_IST.strftime("%Y-%m"))
os.makedirs(MONTH_FOLDER, exist_ok=True)

# file: daily/logs/2026-01/monthlylogs.json
DAILY_LOG_FILE = os.path.join(MONTH_FOLDER, "monthlylogs.json")

# bucket parts
DATE_KEY = NOW_IST.strftime("%d/%m/%Y")

minute = NOW_IST.minute
hour = NOW_IST.hour + (1 if minute > 45 else 0)

bucket_time = (NOW_IST.replace(minute=0, second=0, microsecond=0)
               .replace(hour=hour if hour < 24 else 0))

TIME_KEY = bucket_time.strftime('%I%p')   # e.g. 05PM
...
for movie, d in movies_summary.items():
    if movie not in daily_logs:
        daily_logs[movie] = {}

    if DATE_KEY not in daily_logs[movie]:
        daily_logs[movie][DATE_KEY] = {}

    daily_logs[movie][DATE_KEY][TIME_KEY] = [
        d["shows"],
        d["gross"],
        d["sold"],
        round(d["occupancy"], 2),
...
with open(DAILY_LOG_FILE, "w", encoding="utf-8") as f:
    json.dump(daily_logs, f, ensure_ascii=False, separators=(',', ':'))

print(f"📦 Daily monthly log updated → {DAILY_LOG_FILE}")
